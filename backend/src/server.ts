...
app.get('/oauth/zoho/authorize', (_req, res) => {
  ...
  const authUrl = `https://accounts.zoho.com/oauth/v2/auth?response_type=code&client_id=${clientId}&scope=ZohoCRM.modules.all&redirect_uri=${encodeURIComponent(redirectUri)}&access_type=offline&state=${state}`;
